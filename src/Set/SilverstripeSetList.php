<?php

declare(strict_types=1);

namespace Netwerkstatt\SilverstripeRector\Set;

final class SilverstripeSetList
{
    public const CODE_STYLE = __DIR__ . '/../../config/code-style.php';

    public const SS_4_0 = __DIR__ . '/../../config/silverstripe-4-0.php';

    public const SS_4_1 = __DIR__ . '/../../config/silverstripe-4-1.php';

    public const SS_4_2 = __DIR__ . '/../../config/silverstripe-4-2.php';

    public const SS_4_3 = __DIR__ . '/../../config/silverstripe-4-3.php';

    public const SS_4_4 = __DIR__ . '/../../config/silverstripe-4-4.php';

    public const SS_4_5 = __DIR__ . '/../../config/silverstripe-4-5.php';

    public const SS_4_6 = __DIR__ . '/../../config/silverstripe-4-6.php';

    public const SS_4_7 = __DIR__ . '/../../config/silverstripe-4-7.php';

    public const SS_4_8 = __DIR__ . '/../../config/silverstripe-4-8.php';

    public const SS_4_9 = __DIR__ . '/../../config/silverstripe-4-9.php';

    public const SS_4_10 = __DIR__ . '/../../config/silverstripe-4-10.php';

    public const SS_4_11 = __DIR__ . '/../../config/silverstripe-4-11.php';

    public const SS_4_12 = __DIR__ . '/../../config/silverstripe-4-12.php';

    public const SS_4_13 = __DIR__ . '/../../config/silverstripe-4-13.php';

    public const SS_5_0 = __DIR__ . '/../../config/silverstripe-5-0.php';

    public const SS_5_1 = __DIR__ . '/../../config/silverstripe-5-1.php';

    public const SS_5_2 = __DIR__ . '/../../config/silverstripe-5-2.php';

    public const SS_5_3 = __DIR__ . '/../../config/silverstripe-5-3.php';

    public const SS_5_4 = __DIR__ . '/../../config/silverstripe-5-4.php';

    public const SS_6_0 = __DIR__ . '/../../config/silverstripe-6-0.php';

    public const SS_6_0_ADDITIONAL = __DIR__ . '/../../config/silverstripe-6-0-additional.php';

    public const SS_6_1 = __DIR__ . '/../../config/silverstripe-6-1.php';

    public const SS_6_2 = __DIR__ . '/../../config/silverstripe-6-2.php';
}
