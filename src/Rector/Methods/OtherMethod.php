...
final class OtherMethod extends AbstractScopeAwareRector implements ConfigurableRectorInterface
{
    /**
    ...
    public function getRuleDefinition(): RuleDefinition
    {
        return new RuleDefinition(
            'Adds TODO upgrade comments for various method, static, and trait changes.',
            [
...
CODE_SAMPLE,
    ...
    public function configure(array $configuration): void
    {
        ...
        foreach ($configuration as $item) {
            if (!isset($item['c'], $item['n'])) {
                continue;
            }

            $this->changes[] = [
                'c' => (string) $item['c'],
                'm' => isset($item['m']) ? (string) $item['m'] : '',
                'n' => (string) $item['n'],
                'u' => (bool) ($item['u'] ?? false),
            ];
        }
    ...
    {
        return [
            Expression::class, // Covers MethodCall, NullsafeMethodCall, StaticCall
            ClassMethod::class, // Covers method overrides
            TraitUse::class, // Covers trait removals
        ];
    }

    /**
     * @param Expression|ClassMethod|TraitUse $node
     */
    ...
    private function refactorExpression(Expression $expression): ?Node
    {
        ...
        foreach ($this->changes as $change) {
            if ($change['m'] === '') {
                continue; // Skip class/trait level configurations for method calls
            }
    ...
    private function refactorTraitUse(TraitUse $traitUse): ?Node
    {
        $changed = false;

        foreach ($this->changes as $change) {
            // Only evaluate class/trait-level rules for TraitUse nodes
            if ($change['m'] !== '') {
                continue;
            }

            foreach ($traitUse->traits as $traitNameNode) {
                $traitName = $this->getName($traitNameNode) ?? $traitNameNode->toString();

                if ($this->isClassSameOrSubclassOfConfigured($traitName, (string) $change['c'])) {
                    $todoLine = $this->buildTodoLine($change['c'], '', $change['n']);

                    if ($this->appendTodoDocCommentSafely($traitUse, $todoLine)) {
                        $changed = true;
                    }
                }
            }
    ...
    private function matchesCallTarget(MethodCall|NullsafeMethodCall|StaticCall $call, array $change): bool
    {
        if ($call instanceof StaticCall) {
            ...
            if ($classNode instanceof Name) {
                $className = $this->getName($classNode) ?? $classNode->toString();
                return $this->isClassSameOrSubclassOfConfigured($className, (string) $change['c']);
            }
            return false;
        }
    ...
    {
        if ($methodName === '') {
            return sprintf('@TODO SSU RECTOR UPGRADE TASK - %s: %s', $className, $note);
        }
