...
final class UseInstead extends AbstractScopeAwareRector implements ConfigurableRectorInterface
{
    /**
    ...
    public function getRuleDefinition(): RuleDefinition
    {
        return new RuleDefinition(
            'Adds TODO upgrade comments and attempts to auto-fix methods that should use an alternative instead.',
            [
...
$value = $service->newMethod($arg);
CODE_SAMPLE,
                    [['c' => 'SomeClass', 'm' => 'oldMethod', 'n' => 'use newMethod() instead.', 'u' => false]]
                ),
            ]
        );
    }
    ...
    {
        // Strictly matches "use newMethod instead" or "use newMethod() instead."
        // Fails on anything containing spaces, slashes, operators, or additional sentences.
        if (preg_match('/^use ([a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)(?:\(\))? instead\.?$/i', trim($note), $matches)) {
            return $matches[1];
        }
